### Disciplinas
- Ecossistemas de Startups
- Direito Digital e LGPD
- Fundamentos de Engenharia de Software
- Metodologias Ágeis em Gestão de Projetos
- Desenvolvimento de Software Integrado – DevOps
- Design da Experiência do Usuário
- Controle de Versão e Gerenciamento de Configuração
- Gerenciamento de Produtos
- Integração e Entrega Contínua
- Orquestração de Contêineres e Gerenciamento de Cluster
- Infraestrutura Automatizada
- Desenvolvimento de Software Seguro – DevSecOps
- Testes Automatizados e Contínuos
- Arquitetura de Microsserviços e Escalabilidade
- Documentação Técnica
- Computação em Nuvem
- Computação sem Servidores
- Monitoramento e Análise de Logs
- Tópicos Avançados em Engenharia de Software

## Entregáveis
- [x] Plano de desenvolvimento do projeto (README.md)
- [x] Documentação do projeto (README.md)
- [x] Contexto do negócio (2 a 3 linhas)
- [x] SLA desejado
- [x] Stack proposta (tecnologias, frameworks, linguagens, etc)
- [x] Diagrama de arquitetura do projeto (mermaid, flowchart, etc)
- [x] Marcar quais das 19 disciplinas são mais criticas

--- 
## Entregáveis do projeto 
- [ ] Repositório do projeto (GitHub, GitLab, Bitbucket, etc)
- [ ] Estrutura de pipelines inicial (CI)

--- 
## Como apresentar 

- Cada equipe deve apresentar seu problema e a solução pensada.
- 20min no maximo
