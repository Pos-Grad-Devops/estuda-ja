...
export function HomePage() {
  ...
  const [liveAulas, setLiveAulas] = useState<Aula[]>([])
  ...
  // Destaque de transmissão = api.live (LiveStatus), não Aula.status CRUD.
  useEffect(() => {
    let cancelled = false
      ...
      try {
        const flags = await Promise.all(
          aulas.map(async (aula) => {
            try {
              const live = await api.live.get(aula.id)
              return live.status === 'ao_vivo' ? aula : null
            } catch {
              return null
            }
          }),
        ...
        if (!cancelled) {
          setLiveAulas(flags.filter((aula): aula is Aula => aula != null))
        }
      } finally {
  ...
  return (
    <section>
      <PageHeader
        ...
        title="Sua escola"
        description="Catálogo de cursos e o que está em transmissão ao vivo agora. Abra um curso para ver a trilha de aulas."
        actions={
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2">
        ...
        <FadeContent delay={80} className="rounded-2xl border border-border bg-surface p-5">
          <p className="text-sm text-muted">Em transmissão agora</p>
          <p className="mt-2 font-display text-4xl font-semibold text-live">
            {loading || liveLoading ? '—' : <CountUp to={liveAulas.length} />}
          </p>
        </FadeContent>
      </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted">
                {nextLive.curso?.titulo ?? `Curso #${nextLive.curso_id}`} ·{' '}
                {formatDateBR(nextLive.agendada_em)}
              </p>
            </div>
